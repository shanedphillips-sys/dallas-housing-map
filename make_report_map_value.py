"""
Report map: City of Dallas parcels colored by TOTAL property value per acre (same
data, bins and palette as the webmap's 2D value layer). Styled after the GDPC
generate_zoning_map_north_up.py:
  - north-up / true proportions (EPSG:2276), equal aspect
  - same clipped city boundary as make_report_map_streets.py (black 0.5 pt) with the
    Lake Ray Hubbard exclave AND the severed corridor stub trimmed off
...
Legend per spec: title "Property value per acre" (no "(2D)"); low bucket is "< $100k".

Output: report_map_value.png (repo) + a copy in the GDPC report-charts folder.
"""
import os
import shutil
...
# Value-per-acre bins — match the webmap 2D value layer (VALUE_PER_ACRE_BINS).
VALUE_BINS = [
    (1_000_000,    "#FEE49E", "$100k – $1M"),
    (2_000_000,    "#FEBE8C", "$1M – $2M"),
    (5_000_000,    "#F0827E", "$2M – $5M"),
    (20_000_000,   "#B5435A", "$5M – $20M"),
    (float("inf"), "#7E55B0", "$20M+"),
]
LOW_VALUE = 100_000
MIN_AREA = 100
...
# ---- Parcels colored by total value per acre, kept inside the trimmed city ----
gdfs = [pyogrio.read_dataframe(os.path.join(DATA, f"parcels_{q}.geojson"),
                               columns=["value_per_acre", "area_feet"])
        for q in ["nw", "ne", "sw", "se"]]
parcels = gpd.GeoDataFrame(pd.concat(gdfs, ignore_index=True), geometry="geometry", crs=gdfs[0].crs)
parcels["vpa"] = pd.to_numeric(parcels["value_per_acre"], errors="coerce").fillna(0)
parcels["area"] = pd.to_numeric(parcels["area_feet"], errors="coerce").fillna(0)
parcels = parcels[(parcels["vpa"] >= LOW_VALUE) & (parcels["area"] >= MIN_AREA)].to_crs(PROJ)
...
parcels["color"] = parcels["vpa"].apply(bin_color)
...
leg = ax.legend(handles=handles, loc="upper right", fontsize=16.7, title="Property value per acre",
